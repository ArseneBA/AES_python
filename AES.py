class AES:
    ...
    def key_expansion(self, key):
        Nk = len(key) // 4
        Nb = 4
        Nr = 10 if Nk == 4 else 12 if Nk == 6 else 14  # In our case it's 10
        w = [key[4 * i:4 * (i + 1)] for i in range(Nk)]
        i = Nk
        while i < Nb * (Nr + 1):
            temp = w[i - 1]
            if i % Nk == 0:
    ...
    def mix_columns(self, state):
        res = [[0 for i in range(4)] for j in range(4)]
        for col in range(4):
            res[0][col] = self.multiply_2(state[0][col]) ^ self.multiply_3(state[1][col]) ^ state[2][col] ^ state[3][
                col]
            res[1][col] = state[0][col] ^ self.multiply_2(state[1][col]) ^ self.multiply_3(state[2][col]) ^ state[3][
                col]
            res[2][col] = state[0][col] ^ state[1][col] ^ self.multiply_2(state[2][col]) ^ self.multiply_3(
                state[3][col])
            res[3][col] = self.multiply_3(state[0][col]) ^ state[1][col] ^ state[2][col] ^ self.multiply_2(
                state[3][col])
        return res
